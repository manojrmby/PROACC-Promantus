...
(function (factory) {
    ...
    DmUploader.prototype.methods = {
        start: function (id) {
            ...
            console.log('Inside Dmuploader');
            console.log(id);
            
            if (id == 8) {
                if (!Validate(this)) {
                    Notiflix.Report.Warning('Notification Info', '"Kindly check for duplicate file or File Missing....!"', 'Click');
                    $('#drag-and-drop-zone', '#drag-and-drop-zone1').dmUploader('cancel');
                    return;
                }
            } else if (id == 6) {
                if (!Validate1(this)) {
            ...
            if (this.queue.length != id) {
                Notiflix.Report.Warning('Notification Info', '"Kindly add / Remove files and try again....!"', 'Click'); 
                //alert('Add all the files and try again..!');
                $('#drag-and-drop-zone','#drag-and-drop-zone1').dmUploader('cancel');
                return;
            }
            //if (this.queue.length != 7) {
            //    Notiflix.Report.Warning('Notification Info', '"Kindly add missing files and try again....!"', 'Click'); 
            //    //alert('Add all the files and try again..!');//Manoj
...
function Validate(info) {
    ...
    for (var i = 0; i < a.length; i++) {
        ...
        var file_name_string = a[i].data['name'];
        var file_name_array = file_name_string.split(".")[0];
        NameList.push(file_name_array);


        //NameList.push(a[i].data['name']);
    }
    var status = updateColor(NameList);
    return status
   // Compare(NameList, DefaultList);
}
function updateColor(list) {
    ...
    for (var j = 0; j < list.length; j++) {
        var text = list[j].toLocaleUpperCase();
        var id = text.substring(0, 3);
        $('#' + id).removeClass('bg-danger').addClass('bg-primary');
    }
    status = true;
...
function Validate1(info) {
    ...
    var status = updateColorSAP(NameList);
    return status;
}

function updateColorSAP(list) {
    var status = false;
    var DefaultList = ['RFC_FM_Output', 'BillingDocuments', 'ComplexityAnalysis', 'OrderDocuments', 'SalesDocuments','MaterialityScore'];
    for (var i = 0; i < DefaultList.length; i++) {
        var text = DefaultList[i].toLocaleUpperCase();
        var id = text.substring(0, 3);
        $('#' + id).addClass('bg-danger');
...
//function Compare(NameList, DefaultList) {
//    var arr1 = NameList.map(f => f.toLowerCase());
//    var arr2 = DefaultList.map(f => f.toLowerCase());
...
//        about = true;
//        for (var j = 0; j < arr1.length && about; i++) {
//            if (arr2[i] == arr1[j]) {
//                given.push(arr2[i]);
//                about = false;
//            } else {
